...
export function storyTalkHref(seriesId: string, subjectUserId: string | null): string {
  return subjectUserId == null ? `/app/series/${seriesId}/handoff` : `/app/series/${seriesId}/interview`;
}
    ...
    <div className="pointer-events-none fixed inset-x-0 bottom-0 z-30 flex justify-center pb-[max(22px,env(safe-area-inset-bottom))] lg:hidden">
      <div className="pointer-events-auto flex items-center gap-1 rounded-pill bg-dark p-1.5 shadow-pop">
        <Link
          ...
          className="max-w-[128px] truncate pl-3 pr-1.5 text-[12.5px] font-medium text-[rgba(240,237,230,0.85)] hover:text-paper hover:no-underline"
        >
          {title}
        </Link>
        <Link
