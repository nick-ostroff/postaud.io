...
/**
 * The current user's headshot on the profile screen. Always editable (it's
 * your own photo): the avatar becomes a button that opens a file picker +
 * circular cropper and POSTs the result to `/api/profile/photo`, refreshing
 * the page so the nav and sidebar pick up the new photo. Mirrors
 * `SeriesPhotoEditor`.
 */
export function ProfilePhotoEditor({ name, photoUrl }: { name: string; photoUrl: string | null }) {
  ...
  return (
    <>
      ...
      >
        <Avatar name={name} size="lg" tone="warm" src={photoUrl} />
